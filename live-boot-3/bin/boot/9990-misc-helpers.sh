...
## Patched for version: live-boot-4.0~alpha18

## New cmdline parameter " basemountmode=rw"
## Overrides the live-media partition default RO mount
##
## Then the partition containing the live-media will be writable
## (Experimental): Persistent loopback files in the same partition might work
##
## If " basemountmode=rw" is not specified:
## No changes will be made to original defaults (ro,noatime)

if cat /proc/cmdline|grep -q " basemountmode=rw" ; then
